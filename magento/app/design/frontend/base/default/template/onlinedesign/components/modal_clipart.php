            ...
            <div class="modal-body">
                <div id="nbdesigner_art_container">
                    <span class="view-thumb nbdesigner_thumb" width="100" ng-repeat="art in arts | filterCat : curentCatArt | filter : artName| limitTo : artPageSize">
                        <img title = "{{art.name}}" style="max-width: 100px; max-height: 100px;" class="img-responsive img-thumbnail nbdesigner_upload_image shadow hover-shadow" ng-src="{{art.url}}" ng-click="addArt(art)"  spinner-on-load/>
                    </span>
                </div>
                <div>
                    <button ng-show="(countArt > 10) && (countArt > artPageSize)" style="margin-right: 15px; margin-top: 10px;" id="art-load-more" type="button" class="btn btn-primary shadow nbdesigner_upload" ng-click="loadMoreArt()"><?php echo $this->__("More") ?></button>
