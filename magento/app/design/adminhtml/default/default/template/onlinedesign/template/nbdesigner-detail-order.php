...
if(isset($order_id)){
	...
	if(isset($product_id)){
		...
		foreach($orderItems as $item){
			if($item->getNbdesignerJson() != null || $item->getNbdesignerJson() != "" && $item->getNbdesignerPid() == $product_id){
		...
		foreach ($list_images as $img){
			...
			$frame = $_frame[1];
			$path_dir = explode('media', $img);
			$list_design[$frame] = NBDESIGNER_BASE_URL.'/media/'.$path_dir[1];
		}
	}
}
?>
...
<div id="nbdesign-order-tabs">
    <h2>
        <?php echo $this->__('Detail product design'); ?>
        <a class="button-primary nbdesigner-right" href="<?php //echo admin_url('post.php?post=' . absint($order_id) . '&action=edit'); ?>"><?php //echo $this->__('Back to order', 'nbdesigner'); ?></a>
    </h2>
    <?php //if(!isset($_GET['product_id'])): ?>
            <p><?php echo $this->__('Go to order detail and choose product design you want to view detail!'); ?></p>
    <?php //else: 

        ?>
        <ul class="nbd-nav-tab">
            <li><a href="#customer-design"><?php echo $this->__('Customer design') ?></a></li>
            <li><a href="#save-to-pdf"><?php echo $this->__('Create PDF') ?></a></li>
        </ul>   
                    ...
                    <tbody>
                        <tr valign="top">
                            <th scope="row" class="titledesc">
                                <label><?php echo $this->__('Force all side in the same format'); ?></label>
                            </th>
                            <td class="forminp forminp-text">
                                ...
                                <br /><small><?php echo $this->__('Create a PDF file contain all page in the same page size. By default, each page correspond a PDF file.'); ?></small>
                            </td>
                        </tr>                       
                    </tbody>     
                </table>    
        ...
        <div class="inner side" id="side-<?php echo $key; ?>">
            <table class="form-table">
                <tbody>
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label for="format"><?php echo $this->__('Paper Size'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                        <select name="pdf[<?php echo $key; ?>][format]" id="format" style="width: 25em;">    
                            <option value="-1"><?php printf(__('As product size %s x %s (exclude margin)'), round($pdfData['product_width'] * $unitRatio, 2),  round($pdfData['product_height'] * $unitRatio, 2)); ?></option>
                    <?php
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc">  
                            <label for="orientation"><?php echo $this->__('Product size'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                            <?php echo $this->__('Width'); ?>: <input name="pdf[<?php echo $key; ?>][product-width]" type="text" readonly style="width: 60px;" value="<?php echo round($proWidth * $unitRatio, 2); ?>"> 
                            <?php echo $this->__('Hight'); ?>: <input name="pdf[<?php echo $key; ?>][product-height]" type="text" readonly style="width: 60px;" value="<?php echo round($proHeight * $unitRatio, 2); ?>"> 
                            <br /><small><?php echo $this->__('In mm'); ?></small>
                        </td>                        
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label for="orientation"><?php echo $this->__('Orientation'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                            <select name="pdf[<?php echo $key; ?>][orientation]" id="orientation">
                                <option value="P" selected="selected"><?php echo $this->__('Portrait');?></option>
                                <option value="L"><?php echo $this->__('Landscape');?></option>
                            </select>
                        </td>
                    </tr>                    
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            ...
                            <p><small><?php echo $this->__('Margin top, right, bottom, left in mm'); ?></small></p>
                        </td>
                    </tr>  
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label><?php echo $this->__('Show Bleed Line'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                            <p>
                                <input name="pdf[<?php echo $key; ?>][show-bleed-line]" type="radio" value="yes" onclick="showSettingBleed(this)"><?php echo $this->__('Yes'); ?>
                            </p>   
                            <p>
                                <input name="pdf[<?php echo $key; ?>][show-bleed-line]" type="radio" value="no" checked="checked" onclick="showSettingBleed(this)"><?php echo $this->__('No'); ?>
                            </p>
                        </td>
                    </tr>   
                    <tr valign="top" style="display: none;" class="setting-bleed">
                        <th scope="row" class="titledesc">
                            <label><?php echo $this->__('Margin bleed'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                            ...
                            <p><small><?php echo $this->__('Margin bleed top, right, bottom, left in mm'); ?></small></p>
                        </td>
                    </tr>     
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label><?php echo $this->__('Background'); ?></label>
                        </th>
                        <td class="forminp forminp-text">                            
                            <?php if($pdfData['bg_type'] == 'image'): ?>
                            <a class="button change-background-image" onclick="openImagesPopup(this, 'img-area');  /* changeBackground(this) */"><?php echo $this->__('Change image'); ?></a>
                            <img class="bg_src" src="<?php echo $pdfData['img_src']; ?>"  style="width: 30px; height: 30px; display: inline-block; vertical-align: top;"/><br />
                            <small><?php echo $this->__('Change image with heigh resolution if you want. Support: jpeg, jpg, png, svg'); ?></small>
                            <?php elseif($pdfData['bg_type'] == 'color'): ?>
                            <span 
            ...
            <p><b><?php echo $this->__('Scale Factor').': '.$scale; ?></b></p>
        ...
        <p class="result-pdf">
            ...
            <a href="javascript:void(0)" class="button-primary" id="create_pdf"><?php echo $this->__('Create PDF'); ?></a>
        </p>            
        </div>    
        <script>
            ...
            jQuery(document).ready(function() {
                ...
                var ajaxurl = "<?php echo $this->getUrl('onlinedesign/index/nbdesigner_get_product_info', array('_current'=>false))  ?>"
                jQuery('#create_pdf').on('click', function(e){
                   ...
                   /*  swal({
					...
					swal({
                        title: "<?php echo $this->__('Create PDFs'); ?>",
...
<?php
include_once('modal_gallery_pdf.php');
?>