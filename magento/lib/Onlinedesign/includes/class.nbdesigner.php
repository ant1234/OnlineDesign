    ...
    public function nbd_save_customer_design(){      
        ...
        if (!count($data_after_save_image['mes'])) {              
            ...
            if(($task == 'create_template' || $task == 'edit_template')){
				...
				/* fix code */
				$path_old = getDesignPath($order_id, $product_id);
...
CREATE TABLE {$wpdb->prefix}nbdesigner_templates ( 
 ...
 PRIMARY KEY  (id) 
	...
	private function nbdesigner_insert_table_templates($product_id, $folder, $priority, $publish = 1, $private = 0){
		...
		if($_GET['priority'] == "primary") {
			$folder = "primary";
			$priority = "1";
		}
		// Concatenated with . for readability
		$query = "insert into {$table_name} "
			   . "(id, product_id, folder, user_id, created_date, publish, private, priority) values "
			   . "(null, '".$product_id."', '".$folder."', '".$user_id."', '".$created_date->format('Y-m-d H:i:s')."', '".$publish."', '".$private."', '".$priority."')";
            ...
            if (move_uploaded_file($val["tmp_name"],$full_name)) {
                if($key != 'used_font' && $key != 'config' && $key != 'design'){
					...
					if ($type == IMAGETYPE_PNG) {
						...
						$scaleQuality = round(($_quality/100) * 9);
						// *** Invert qualit setting as 0 is best, not 9
						$invertScaleQuality = 9 - $scaleQuality;
						
						$image_resized = $this->nbdesigner_resize_imagepng($full_name, $_width, $_height);
						imagepng($image_resized, $thumb_file, $invertScaleQuality);
					} elseif ($type == IMAGETYPE_JPEG) {
						$image_resized = $this->nbdesigner_resize_imagejpg($full_name, $_width, $_height);
						imagejpeg($image_resized, $thumb_file, $_quality);
					}
					$links[$key] = Nbdesigner_IO::secret_image_url($thumb_file);	
    ...
    public function nbdesigner_order_item_name($item_name, $item){       
        ...
        if(isset($order_id[0]) && ($_show_design_order == 'yes')){ 
            $notice = '';
            $html = '';
            $oiid = '';
            ...
            if(isset($item["item_meta"]["_nbdesigner_folder_design"])){
                ...
                if(isset($_nbdesign_order_item_id[0])) $oiid = $_nbdesign_order_item_id[0];
            }   
            $redesign_url = getUrlPageNBD('redesign');;
            if(is_array($folder)){
                $link_redesign = $redesign_url.'?product_id='.$item["product_id"].'&orderid='.$order_id[0].'&folder='.$folder[0].'&oiid='.$oiid;
            }else{
                $link_redesign = $redesign_url.'?product_id='.$item["product_id"].'&orderid='.$order_id[0];
            }  
            if(isset($design_data[$index]) && ($design_data[$index] == 'decline')){
                $notice = "<small style='color:red;'>". __('(Rejected! Click ', 'nbdesigner')."<a href='".$link_redesign."' target='_blank'>". __('here ', 'nbdesigner'). "</a>". __(' to design again', 'nbdesigner')."!)</small>";
            }
            if(isset($design_data[$index]) && ($design_data[$index] == 'accept')){
                $notice = __('<small> (Approved!)</small>', 'nbdesigner');
            }
    ...
    public function nbdesigner_get_qrcode(){
        ...
        if(isset($_REQUEST['data'])){
            ...
            if($this->nbdesigner_save_data_to_image($full_name, $image)){
                $result['flag'] = 1;
				/* fix code NBDESIGNER_DATA_URL.'/temp/'.$new_name; */
                $result['src'] =  NBDESIGNER_DATA_URL.'/temp/'.$file_name;
            };          
        }
        echo json_encode($result);
        wp_die();
    ...
    public function nbdesigner_copy_image_from_url(){
		...
		/* fix code */
        $res['src'] = NBDESIGNER_DATA_URL.'/temp/'.$new_name;
        if(@copy($url, $path)){
            $res['flag'] = 1;
        } else {
            $res['flag'] = 0;
    ...
    public function nbdesigner_save_design_to_pdf(){       
		...
		require_once Mage::getBaseDir('lib') .DS. 'Onlinedesign'.DS.'includes'.DS.'tcpdf'.DS.'tcpdf.php';
        ...
        foreach($pdfs as $key => $_pdf){
            ...
            if($bg_type == 'image'){
                ...
                if($check_img){
					...
					//$pdf->Image('http://productdesign.com/media/nbdesigner/2017/03/6_2_1.png', $mLeft, $mTop, $bgWidth, $bgHeight, '', '', 'http://www.tcpdf.org', true, '');
