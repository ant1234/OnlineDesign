...
angular.module("app").controller("DesignerController", function($scope, PatternService, ProductService, localStorageService, FontService, ArtService, QrcodeService, $interval, $upload, $timeout, LanguageService, AdminTemplateService) {
    ...
    $scope.background = 'CCCCCC'; /* NBDESIGNCONFIG['nbdesigner_default_color'].substr(1, 7); */
    ...
    $scope.shadow.color = 'CCCCCC'; /* NBDESIGNCONFIG['nbdesigner_default_color'].substr(1, 7); */
    $scope.loaded = 0;
    $scope.zoom = 1;
    $scope.settings = {};
    $scope.Popover = "none";
    ...
    $scope.initStage = function(data){
        ...
        if (toType(data.design) == "object"){
            ...
            if ((toType(localStorageService.get(sid)) === "object") && (NBDESIGNCONFIG['task'] == "") && !$scope.ignoreLocalDesign && !NBDESIGNCONFIG['is_designer']){
    ...
    $scope.loadAdminListDesign = function(){
        if(! _.size($scope.adminListTemplate)){
            show_indicator();
            AdminTemplateService.getDesign(NBDESIGNCONFIG['product_id']).then(function(data) {                
                if(data['mes'] == 'success'){
    ...
    $scope.loadExtraAdminDesign = function(id){
        ...
        $scope.canvas.clear();
        $scope.loadProduct(NBDESIGNCONFIG['product_id'], 0, id);
    };
    ...
    $scope.fitReferenceDesign = function(id){
        if(NBDESIGNCONFIG['reference_product'] == '') return;
        if(_.size($scope.refProduct)) return;
    ...
    $scope.storeDesign = function() {
        ...
        if(!$scope.partialSave){
            ...
            NBDESIGNERCART.upload_blob($scope.dataCustomerDesign, $scope.canvases, fonts, config);
        }else{
    ...
    $scope.importDesign = function(){
		$('#design-json-content').css("border", "1px solid #ccc");
		if($('#design-json-content').val() == "") {
			$('#design-json-content').css("border", "1px solid red");
			alert("Please push the content");
			return false;
		}
        ...
        $('#dg-expand-feature').modal("hide");
		alert("Your desisn has been applied successfully!");
    };
...
angular.module("app").service("ProductService", function($http, $q) {
    this.getProduct = function(id, variation, template) {
        ...
        var data = "action=nbdesigner_get_product_info&nonce=" + NBDESIGNCONFIG['nonce'] + "&product_id=" + id;
        if(NBDESIGNCONFIG['order_id']) data += "&order_id=" + NBDESIGNCONFIG['order_id'];
        if(template) data += "&template_folder=" + template;
        if(variation) data += '&variation_id=' + variation;
        if(NBDESIGNCONFIG['reference_product']) data += '&reference_product=' + NBDESIGNCONFIG['reference_product'];
        if(NBDESIGNCONFIG['order_item_folder']) data += '&order_item_folder=' + NBDESIGNCONFIG['order_item_folder'];
        $http({
            url: NBDESIGNCONFIG['ajax_url'],
            method: "POST",
            data:     data,
...
angular.module("app").service("AdminTemplateService", function($http, $q) {
    this.getDesign = function(id) {
        ...
        $http({
            url: NBDESIGNCONFIG['ajax_url'],
            method: "POST",
            data: "action=nbdesigner_load_admin_design&nonce=" + NBDESIGNCONFIG['nonce'] + "&id=" + id ,
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            }            
        }).success(function(data, status, headers, config) {
...
var NBDESIGNERCART = {
    ...
    upload_design: function(data_img, data_json, fonts, config) {
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                "image": data,
                "nonce": NBDESIGNCONFIG['nonce'],
                "sid": nbd_window.nbds_frontend.sid,
                "adid": NBDESIGNCONFIG['template_folder'],
                "product_id": NBDESIGNCONFIG['product_id'],
                "orderid": NBDESIGNCONFIG['order_id'],
                "task" : NBDESIGNCONFIG['task'],
                "fonts" : _fonts,
                "config" : _config,
                "priority" : NBDESIGNCONFIG['template_priority'],
                "save_status" : NBDESIGNCONFIG['save_status'],
                "oiid" : NBDESIGNCONFIG['order_item_id'],
                "folder" : NBDESIGNCONFIG['order_item_folder']
            }
        }).done(function(data) {
            ...
            if (_data.flag == "success") {
                ...
                if(NBDESIGNCONFIG['task'] == '' && NBDESIGNCONFIG['order_id'] == ''){
                    nbd_window.NBDESIGNERPRODUCT.get_sugget_design(NBDESIGNCONFIG['product_id']);
                }
                if (_data.redesign != "") nbd_window.hideDesignFrame(_data.redesign);
                else nbd_window.hideDesignFrame()
            } else if (_data.flag == "pendding") {
                if (_data.redesign != "") nbd_window.hideDesignFrame(_data.redesign)
            } else alert("Something wrong! Please try again...");
            NBDESIGNCONFIG['save_status'] = 1;
            NBDESIGNCONFIG['first_time_edit_temp'] = 1;
            hide_indicator()
        })
    },
        ...
        jQuery.ajax({
            ...
            if (_data.flag == "success") {
                ...
                if (self.dataImage[next_key] && self.flag != 0) {
                    ...
                    nbd_window.NBDESIGNERPRODUCT.show_design_thumbnail(self.thumbs);
                    nbd_window.hideDesignFrame();
                    self.resetData();
    ...
    upload_blob : function(data_img, data_json, fonts, config){
        show_indicator(); 
        ...
        var fd = new FormData();
        fd.append('action', 'nbd_save_customer_design');
        fd.append('nonce', NBDESIGNCONFIG['nonce']);
        fd.append('template_folder', NBDESIGNCONFIG['template_folder']);
        ...
        fd.append('priority', NBDESIGNCONFIG['template_priority']);
        fd.append('save_status', NBDESIGNCONFIG['save_status']);
        fd.append('order_id', NBDESIGNCONFIG['order_id']);
        fd.append('order_item_folder', NBDESIGNCONFIG['order_item_folder']);           
...
jQuery(document).ready(function($){
    nbd_window.jQuery('.variations_form.cart').on('change', function(){
        ...
        if(!scope.onLoadProduct && variation_id != ''){
            ...
            show_indicator();
            scope.canvas.clear();
            scope.loadProduct(NBDESIGNCONFIG['product_id'], variation_id,  'primary');              
        }      
        if (scope.$root.$$phase !== "$apply" && scope.$root.$$phase !== "$digest") scope.$apply()
    });
    ...
    nbd_window.jQuery('#nbdesign-new-template').on('click', function(){
        NBDESIGNCONFIG['save_status'] = 0;
        var d = new Date();
        NBDESIGNCONFIG['template_folder'] = d.getTime();
    });    
    nbd_window.jQuery('#triggerDesign').on('click', function(){
        if(NBDESIGNCONFIG['first_time_edit_temp']){
            NBDESIGNCONFIG['save_status'] = 1;
            NBDESIGNCONFIG['template_folder'] = NBDESIGNCONFIG['origin_folder_template'];            
        }
    });    
