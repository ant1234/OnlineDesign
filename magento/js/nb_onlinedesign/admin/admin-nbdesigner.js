...
var NBDESIGNADMIN = {
    ...
    add_google_font: function (e) {
		...
		var _font_exists = false;
		
		jQuery("#nbdesigner_container_list_google_font span a span").each(function(index, elem){
			if(name.trim() === jQuery(this).text().trim()){
				alert("This font exists. Please enter other font");
				_font_exists = true;
				return false;
			}
		});
		
		if(_font_exists) return;
    ...
    init_color_picker: function(){
		...
		jQuery.each(jQuery('.nbd-color-picker'), function () {
			//jQuery(this).addClass("jscolor");
            jQuery(this).keyup(function() {
				var $input = jQuery(this);
				setTimeout(function () {
					if ($input.val().length >= 3) {
						$input.parents('.nbdesigner-box-collapse').find('.nbdesigner-image-original').css("background", ""); 
						$input.parents('.nbdesigner-box-collapse').find('.nbdesigner-image-original').css("background", $input.val()); 
					}
				}, 5);
            });            
        })
